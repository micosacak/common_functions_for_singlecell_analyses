...
# this function is modified to put a ligand at the center of a circle and receptor cells arounf the circle.

netVisual_diffInteraction_circular <- function(object, new_levels, comparison = c(1, 2), measure = c("count", 
    ...
    vertex.label.show = TRUE, edge.weight.max = NULL, edge.width.max = 8, 
    alpha.edge = 0.6, label.edge = FALSE, edge.label.color = "black", 
    edge.label.cex = 0.8, edge.curved = 0.2, shape = "circle", 
    layout = NULL, margin = 0.2, arrow.width = 1, arrow.size = 0.2) 
{
    options(warn = -1)
    ...
    net <- net.diff[new_levels, new_levels]
    ...
    # Custom layout to place the specified source at the center
    custom_layout <- function(g, source_node) {
        layout <- matrix(ncol = 2, nrow = vcount(g))
        source_index <- which(V(g)$name == source_node)
        if (length(source_index) == 0) stop("Source node not found in the graph")
        layout[source_index, ] <- c(0, 0)  # Place the source at the center
        angle <- seq(0, 2 * pi, length.out = vcount(g))
        other_indices <- setdiff(1:vcount(g), source_index)
        layout[other_indices, ] <- cbind(cos(angle[1:length(other_indices)]), sin(angle[1:length(other_indices)]))
        return(layout)
    }
    
    if (is.null(layout)) {
        if (length(sources.use) != 1) stop("Exactly one source must be specified")
        coords <- custom_layout(g, sources.use)
    } else {
    ...
    unique_names <- make.unique(V(g)$name)
    igraph::V(g)$name <- unique_names
    if (length(color.use) != length(unique_names)) {
        color.use <- rep(color.use, length.out=length(unique_names))
    }
    igraph::V(g)$color <- color.use
    igraph::V(g)$frame.color <- color.use
    
    if (vertex.label.show) {
